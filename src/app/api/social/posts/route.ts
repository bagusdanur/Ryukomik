...
export async function POST(request: Request) {
  try {
    ...
    const body=await request.json() as {content?:string;image_url?:string;visibility?:string;parent_id?:string;reply_to_id?:string};
    ...
    let parent:{id:string;author_id:string}|undefined, replyTarget:{id:string;author_id:string;parent_id:string|null}|undefined;
    ...
    if(body.reply_to_id&&parent){const found=await socialQuery<{id:string;author_id:string;parent_id:string|null}>("select id,author_id,parent_id from social_posts where id=$1",[body.reply_to_id]);replyTarget=found.rows[0];if(!replyTarget||(replyTarget.id!==parent.id&&replyTarget.parent_id!==parent.id))return socialJson({error:"Balasan yang dituju tidak ditemukan."},{status:404});await ensureSocialProfile(replyTarget.author_id);}
    const notificationTarget=replyTarget||parent;
    const notify=notificationTarget&&notificationTarget.author_id!==userId?await allowsSocialNotification(notificationTarget.author_id,"replies"):false;
    const post=await socialTransaction(async client=>{const actor=await client.query<{username:string;avatar_url:string|null;level:number;role:string|null;is_premium:boolean}>("select username,avatar_url,level,role,is_premium from social_profiles where user_id=$1",[userId]);const profile=actor.rows[0]||{username:"User",avatar_url:null,level:1,role:null,is_premium:false};const name=profile.username;const inserted=await client.query<{id:string;author_id:string;created_at:string}>("insert into social_posts(author_id,parent_id,content,image_url,visibility) values($1,$2,$3,$4,$5) returning id,author_id,created_at",[userId,parent?.id||null,content,media,visibility]);if(notificationTarget&&notify)await client.query("insert into social_notifications(user_id,actor_id,actor_name,type,target_id) values($1,$2,$3,'social_reply',$4)",[notificationTarget.author_id,userId,name,parent?.id||notificationTarget.id]);await client.query("insert into social_activity_events(actor_id,actor_name,event_type,entity_id,entity_label,visibility) values($1,$2,$3,$4,$5,$6)",[userId,name,parent?"replied_post":"created_post",inserted.rows[0].id,content.slice(0,80),visibility]);return {...inserted.rows[0],profiles:profile};});
