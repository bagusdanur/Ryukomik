...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("user_collections")
      .select("id, name, description, visibility, is_public, created_at, updated_at")
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("user_collections").insert({
      ...
      visibility, is_public: visibility === "public",
    }).select("id").single();
    if (error) throw error;
...
export async function PATCH(request: Request) {
  try {
    ...
    const body = await request.json() as { id?: string; name?: string; description?: string; visibility?: string };
