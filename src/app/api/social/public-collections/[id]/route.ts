...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: collection, error } = await supabaseAdmin.from("user_collections").select("id, user_id, name, description, cover_url, visibility, items_count, updated_at, profiles!user_collections_user_id_fkey(username, avatar_url)").eq("id", id).maybeSingle();
    ...
    return socialJson({ collection, items, nextCursor: rows.length > 20 ? encodeSocialCursor(items.at(-1)?.created_at, items.at(-1)?.slug) : null }, { headers: { "Cache-Control": collection.visibility === "public" ? "public, s-maxage=300, stale-while-revalidate=300" : "private, no-store" } });
