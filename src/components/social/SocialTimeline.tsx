...
function Composer({ parentId, compact = false, onCreated }: { parentId?: string; compact?: boolean; onCreated: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setContent(""); setImage(""); onCreated();
    } catch (failure) {
      setError(failure instanceof Error ? failure.message : "Gagal mengirim postingan.");
...
function PostCard({ post, onChanged }: { post: Post; onChanged: () => void }) {
  ...
  async function toggleLike() {
    const next = !liked; setLiked(next); setLikes((value) => Math.max(0, value + (next ? 1 : -1)));
    try { await socialFetch("/api/social/posts/like", { method: next ? "POST" : "DELETE", body: JSON.stringify({ postId: post.id }) }); }
    catch { setLiked(!next); setLikes((value) => Math.max(0, value + (next ? -1 : 1))); }
  }
  async function openReplies() {
    setReplying((value) => !value); if (loadedReplies) return;
    try { const result = await socialFetch<{ items: Post[] }>(`/api/social/posts?parentId=${post.id}`); setReplies(result.items); setLoadedReplies(true); }
    catch { setReplies([]); }
  }
  ...
  async function report() { const reason = prompt("Alasan laporan:"); if (!reason) return; await socialFetch("/api/social/reports", { method: "POST", body: JSON.stringify({ targetType: "post", targetId: post.id, reason }) }); alert("Laporan dikirim."); }
...
export default function SocialTimeline() {
  ...
  const load = useCallback(async (reset = false) => {
    ...
    try {
      const result = await socialFetch<{ items: Post[]; nextCursor: string | null }>(`/api/social/posts?scope=${scope}${!reset && cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`);
      setItems((current) => reset ? result.items : [...current, ...result.items]); setCursor(result.nextCursor);
    } catch (failure) { setError(failure instanceof Error ? failure.message : "Timeline gagal dimuat."); }
